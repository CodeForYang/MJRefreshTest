...
-(void)setRefreshingTarget:(id)target refreshAction:(SEL)action{
...
#pragma mark - 这个小细节 有空看看???
        //预防正在刷新中时,调用本方法使得header inset回调失败
        if (self.state != MJRefreshStateRefreshing) {
            self.state = MJRefreshStateWillRefresh;
            //刷新(预防从另一个控制器回到这个控制器的情况,回来要重新刷新)
