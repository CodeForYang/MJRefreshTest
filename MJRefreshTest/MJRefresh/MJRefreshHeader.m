...
NSString * const MJRefreshHeaderRefreshing2IdleBoundsKey = @"MJRefreshHeaderRefreshing2IdleBoundsKey";
NSString * const MJRefreshHeaderRefreshingBoundsKey = @"MJRefreshHeaderRefreshingBoundsKey";
...
- (void)placeSubviews{
    ...
    //设置y值(当自己高度发生改变,肯定要重新调整Y值,所以放到placeSubviews中设置Y值)
...
- (void)resetInset{
    if (@available(iOS 11.0,*)) {
        ...
        // 如果 iOS 10 及以下系统刷新时,push 新的 VC,等待刷新完成后回来,会导致Inset.top 异常,不能 resetInset,检查以下这种特殊情况
    ...
    //避免 CollectionView 在使用根据 Autolayout 和 内容自动伸缩 cell,刷新时导致的Layout异常渲染问题
...
- (void)headerEndingAction{
    ...
    //由于修改 Inset 会导致 self.pullingPercent 联动设置 self.alpha, 故提前获取alpha的值,后续用于还原 alpha动画
    ...
    if (self.isAutomaticallyChangeAlpha) {
        ...
        //由于修改了 inset 导致,pullingPercent 被设置值, alpha 已经被提前修改为 0 了,所以这里不用置 0,但为了代码的严谨性,不依赖其他的特殊实现方式,这里还是置0
        self.alpha = 0;
    }
